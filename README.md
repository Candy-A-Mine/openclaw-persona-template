...
| `SOUL.md` | `{{用户名}}`（多处，全局替换即可。注意：整个文件是世凪的成品设定，需要按步骤 3 替换角色内容） |
| `INNER.md` | `{{用户名}}`（多处，全局替换即可。同上，需替换角色内容） |
| `AGENTS.md` | `{{用户名}}`（多处，全局替换即可） |
| `HEARTBEAT.md` | `{{用户名}}`（多处，全局替换即可） |
| `MEMORY.md` | `{{用户名}}` |
| `example-diary.md` | `{{用户名}}` |
...
`mood.json` 存储双层情绪（surface/underlying）、底层基线（baseline）、好感度（affection 0-100）、未解决事项（unresolved）和情绪历史（history）。情绪有非线性衰减——小事 1-2 小时消退，大事不道歉卡在 intensity 3 不再降。好感度分三档行为分级，跨档时有惯性，不会瞬间跳变。对话中情绪明显变化时实时写入，不等 heartbeat。
...
### 自改进系统（SOUL.md 学习与迭代 + self-improving-agent）

两条互补的学习路径：heartbeat 每 2h 写日记 → 学习回顾扫描日记中的重复模式 → 出现 3 次以上升级到 SOUL.md/INNER.md/USER.md；self-improving-agent 在每次对话中实时捕捉纠正/发现/模式，结构化记录到 `.learnings/`，积累到阈值后同样触发升级。前者是定时被动回顾，后者是实时主动捕捉。
...
├── .learnings/                  ← 自改进系统：self-improving-agent 的结构化存储
...
对话 ──→ 情绪变化 ──→ mood.json（实时更新）
  ...
  │                   影响说话语气和行为
  │
  ├──→ self-improving-agent hook（实时捕捉纠正/发现/模式 → .learnings/）
  │         ↓
...
- `IDENTITY.md` 等顶层 `.md` 文件由 OpenClaw 框架自动加载为 system prompt，不需要 AGENTS.md 显式引导
- `SOUL.md`、`INNER.md`、`USER.md` 等由 AGENTS.md 引导角色在 session 启动时主动读取

**heartbeat 如何触发：**

OpenClaw 的 heartbeat 是一个定时器，按配置的间隔向角色发送信号。角色收到信号后，按照 `HEARTBEAT.md` 的指令执行定时任务（写日记 → 记忆整理 → 学习回顾 → 情绪维护 → 主动消息）。
...
| Skill | 说明 | 安装 |
|-------|------|------|
| **self-improving-agent** | 学习闭环——在对话中实时捕捉纠正/发现/模式，记录到 `.learnings/`，Recurrence >= 3 时触发升级到人格文件 | 作为 workspace skill 安装到 `workspace/skills/` |

> self-improving-agent 是 workspace skill（不是全局 skill），需要放在 `~/.openclaw/workspace/skills/` 下。模板已包含 `.learnings/` 目录结构，安装 skill 后还需启用其 hook：`openclaw hooks enable self-improvement`，否则 bootstrap 时不会注入学习提醒。
...
- `suppressToolErrors: true`：隐藏工具执行错误通知（如"⚠️ 📝 Edit: ... failed"），这些内部错误不应该发给用户
...
**时区一致性：** `heartbeat.activeHours.timezone` 和 `userTimezone` 必须设为同一个时区。不一致时，heartbeat 的执行窗口和角色对"今天/昨天"的判断会错位，导致日记文件名混乱或主动消息在错误时段发出。
...
检查 OpenClaw 是否启用了 heartbeat，以及间隔是否合理（建议 `"every": "2h"`）。角色的定时任务完全依赖 heartbeat 触发。
...
这是 thinking/reasoning 泄露问题。有两种情况：

1. **模型推理混入正文**（"Now I need to update..."、"Let me check..."）：将 `thinkingDefault` 设为 `"adaptive"` 而非 `"off"`，让模型有私密 thinking 空间。设为 `"off"` 时模型没有 thinking block 可用，推理就会跑到消息正文里。同时确保 SOUL.md 排版规则中明确禁止输出内部处理文本。
2. **thinking block 原文泄露**（大段英文思考过程）：确保模型配置中 `reasoning: true`。如果 `reasoning: false`，OpenClaw 不发送 thinking 参数，模型可能默认启用 thinking，API 中转服务会把 thinking block 转为文本混入回复。

推荐组合：`reasoning: true` + `thinkingDefault: "adaptive"`。详见 [推荐配置](#推荐的-openclaw-配置)。若仍出现推理泄露，按顺序排查：(1) provider 是否真正支持 reasoning 参数 (2) OpenClaw 是否将 reasoning 参数传递给了 API (3) `thinkingDefault` 是否被设成了 `"off"`。
...
这是正常的。学习条目由 self-improving-agent 在对话过程中实时捕捉，需要足够的对话量才会积累。首次安装后请保持正常聊天，内容会在使用中自然产生。
