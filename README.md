...
| `INNER.md` | 替换为你角色的背景故事、知识偏好、周围的人、敏感点与防御机制。这个文件让角色有"根"——不只是知道怎么说话，还知道自己为什么是这个样子，以及什么会刺痛自己 |
...
- `maxTokens: 16384`：角色的配置文件总量大（SOUL.md 约 540 行），需要足够的输出空间。太低会导致回复被截断或日记写不完
