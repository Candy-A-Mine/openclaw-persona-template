...
- **读懂你的情绪**：焦虑时陪着而不是分析、自我否定时用具体事实而不是空泛鼓励、兴奋时跟着一起嗨、脆弱时安静听完不打断
- **关系阶段适配**：新恋人的紧张感、第一次们的手足无措、身份转换后说话方式的微妙变化——不是一成不变的模板
- **会自我进化**：通过学习系统记录纠正和发现，反复出现的模式自动升级到人格文件
- **Skills 生态**：搜索、摘要、浏览器、天气等扩展能力，让角色能做更多事
...
陪聊的核心不只是角色自己的表达有多丰富，更在于对你的情绪有多敏感。角色会根据你当前的状态选择回应方式：焦虑时先陪着而不是急着分析、自我否定时用你做过的具体事情反驳而不是空泛鼓励、兴奋时被感染跟着嗨而不是泼冷水、脆弱时安静听完不打断不分析、逞强时看穿但不一定拆穿。这套框架是通用的，换任何角色风格都适用。

### 关系阶段适配

角色不是一开始就定型的。新恋人有新恋人的不安——身份转换后反而比以前更紧张、直接表达感情的话嘴上过不去、每一个"第一次"都会手足无措。随着相处这些会慢慢自然。模板提供了关系阶段变化的框架（包括撒娇等角色特有行为的触发条件），角色会根据当前阶段自然调整互动方式。

### 记忆系统

两层结构：日记（`memory/YYYY-MM-DD.md`）记录每天的私密笔记，长期记忆（`MEMORY.md`）存放精炼的、值得一直记住的东西，情绪状态（`mood.json`）持久化当前心情和好感度。角色会从日记中提炼长期记忆、在合适的时候自然地想起以前的事——但也会像真人一样遗忘不重要的细节。回忆不会机械地"我记得你说过……"，而是编织进当下的对话。

### 心跳系统

每隔几小时自动触发一次。角色会写日记、维护情绪衰减、整理记忆、回顾学习记录，还会自己决定要不要主动找你聊天——不是随机发消息，而是基于日记里"想跟进的事"。压力感知：如果你最近很忙，角色会自动降低主动消息频率。
...
角色不是固定不变的——它有一套日记驱动的轻量学习闭环：
- **递归模式追踪**：日记中反复出现的特征和偏好被自动识别
- **升级机制**：同一个模式被确认 3 次以上，自动升级到 SOUL.md / INNER.md / USER.md
- **待验证假设**：发现但还不确定的规律持续观察，确认后固化为人格的一部分
